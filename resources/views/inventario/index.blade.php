...
<div class="container" style=" min-height: 75vh;display: flex; flex-direction: column;">
    ...
    <div class="wrap d-flex flex-wrap justify-content-start">
        ...
        <div class="box one m-2" onclick="window.location.href='{{ route('hardwares.index', ['categoria_id' => $categoria->id]) }}';">
    ...
    <div class="mt-auto">
            <div class="row justify-content-between align-items-center mt-3">
                <div>
                    Mostrando registros del {{ $categorias->firstItem() }} al {{ $categorias->lastItem() }} de un total de {{ $categorias->total() }} registros
                </div>
                @if($categorias->total() > $categorias->perPage())
                <div>
                    {{ $categorias->links() }}
                </div>
                @endif
