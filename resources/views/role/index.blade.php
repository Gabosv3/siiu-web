    ...
    <nav>
        <div class="nav nav-tabs" id="nav-tab" role="tablist">
            ...
            <button class="nav-link" id="nav-profile-tab" data-bs-toggle="tab" data-bs-target="#nav-profile" type="button"
                role="tab" aria-controls="nav-profile" aria-selected="false">Desactivados</button>
        </div>
    </nav>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @if ($role->name !== 'SuperAdmin')
                                        ...
                                        @can('role.edit')
                                            <a title="Editar rol" href="{{ route('role.edit', $role->id) }}"
                                                class="btn btn-green-600 mb-3" id="btn-redireccionar-editar-rol"><i
                                                    class='bx bxs-edit-alt'></i></a>
                                        @endcan
                                        <!-- Botón para eliminar el rol (visible solo para usuarios con permiso) -->
                                        @can('role.destroy')
                                            <form method="POST" class="formulario-eliminar" style="display:inline;"
                                                action="{{ route('role.destroy', $role->id) }}">
                                                @method('DELETE')
                                                @csrf
                                                <button title="Eliminar rol" class="btn btn-red-800"><i class='bx bxs-trash'
                                                        id="btn-eliminar-rol"></i></button>
                                            </form>
                                        @endcan
                                        <button title="Clonar rol" class="btn btn-info mb-3 btn-clonar-rol"
                                            data-role-id="{{ $role->id }}" data-role-name="{{ $role->name }}"
                                            data-bs-toggle="modal" data-bs-target="#modalClonarRol">
                                            <i class='bx bxs-copy'></i>
                                        </button>
                                    @else
                                        <!-- Botones desactivados o no visibles para el rol "SuperAdmin" -->
                                        <button class="btn btn-green-600 mb-3" disabled><i
                        ...
                        @foreach ($rolesDelets as $key => $role)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @can('role.restore')
                                        <form action="{{ route('role.restore', $role->id) }}" class="formulario-restaurar"
                                            method="POST">
                                            @csrf
                                            @method('PUT')
                                            <button class="btn btn-cyan-800 mb-3" type="submit"
                                                id="btn-restaurar-rol">Restaurar</button>
                                        </form>
                                    @endcan
                                </td>
                            </tr>
