    ...
    <table id="Principal" class="table align-items-center mb-0 text-center" style="width:100%">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    @if ($status == 'inactive')
                    <form action="{{ route('departamentos.restore', $department->id) }}" method="POST" style="display:inline;" class="formulario-restaurar">
                        @csrf
                        @method('PUT')
                        <button type="submit" class="btn btn-cyan-800">Restaurar</button>
                    </form>
                    @else
                    <a href="{{ route('departaments.show', $department->id) }}" title="Ver departamento" class="btn btn-cyan-800"><i class='bx bxs-show'></i></a>
                    <a href="{{ route('departaments.edit', $department->id) }}" title="Editar departamento" class="btn btn-green-600"><i class='bx bxs-edit-alt'></i></a>
