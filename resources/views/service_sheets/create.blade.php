...
<div class="container">
    ...
    <div id="sheets-container">
        <form id="supplyForm" action="{{ route('service_sheets.store') }}" method="POST">
            @csrf
            <div class="card sheet-entry mb-3" data-sheet-id="1">
                <div class="card-body">
                    ...
                    <!-- Contenido de la hoja de servicio -->
                    <div class="row">
                        @php
                        // IDs
                        ...
                        $usuarioNombre = $data->ticket->user->name;
                        $tecnicoNombre = $data->technician->user->name;
                        @endphp

                        @foreach ([['label' => '📅 Fecha', 'name' => 'date', 'value' => '', 'class' => 'date-value', 'hidden' => true],
                        ['label' => '🏢 Unidad/Depto.', 'name' => 'department_id', 'value' => $departamentoId, 'display' => $departamentoNombre],
                        ['label' => '👤 Usuario', 'name' => 'user_id', 'value' => $usuarioId, 'display' => $usuarioNombre],
                        ['label' => '🛠 Técnico', 'name' => 'technician_id', 'value' => $tecnicoId, 'display' => $tecnicoNombre]] as $field)
                        <div class="col-md-3 col-12 mb-3">
                            <div class="info-box">
                                <span class="info-label">{{ $field['label'] }}</span>
                                <span class="info-value {{ $field['class'] ?? '' }}">{{ $field['display'] ?? $field['value'] }}</span>
                    ...
                    <div class="row mt-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="hardware-list" name="hardware_id" class="form-control hardware-list">
                    ...
                    <div class="row mt-3">
                        <div class="col-12">
                            <button type="button" class="btn btn-success add-supply">Agregar Insumo</button>
                        </div>
                    </div>

                    <input type="hidden" name="supplies_data" id="suppliesData">
    ...
    document.addEventListener("DOMContentLoaded", function() {
            ...
            if (categoryId) {
                $.get(`/service-sheets/get-hardware/${categoryId}`, function(data) {
                    ...
                    if (data.length > 0) {
                        data.forEach(item => {
                            hardwareList.append(
                                `<option name="hardware_id" value="${item.id}">${item.inventory_code}</option>`
                            );
                        });
                    } else {
                        hardwareList.append(
        ...
        $(document).on("change", ".category-supplies", function() {
            ...
            if (categoryId) {
                $.get(`/service-sheets/get-supplies/${categoryId}`, function(data) {
                    ...
                    if (data.length > 0) {
                        data.forEach(item => {
                            supplyList.append(`<option value="${item.id}">${item.name}</option>`);
                        });
                    } else {
                        supplyList.append('<option value="">No hay insumos disponibles</option>');
                    }
                }).fail(() => {
        ...
        function updateSupplyTable(sheetEntry, sheetId) {
            ...
            <tr>
                ...
                <td><button type="button" class="btn btn-danger btn-sm remove-supply" data-sheet-id="${sheetId}" data-index="${index}">Eliminar</button></td>
            </tr>
        `);
            });
        }
        ...
        $("#supplyForm").on("submit", function(event) {
            ...
            Object.keys(selectedSupplies).forEach(sheetId => {
            ...
            // Guardar los datos en el campo oculto en formato JSON
            $("#suppliesData").val(JSON.stringify(suppliesData));
        });
