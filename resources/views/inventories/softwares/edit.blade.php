...
<div class="container card" style="height-min: 75vh ;">
    ...
    <form action="{{ route('softwares.update', $software->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <div class="d-flex align-items-center">
                ...
                <select id="fabricante_id" name="manufacturer_id" class="form-control js-select-fabricante" style="width: 90%;">
                    ...
                    <option value="{{ $fabricante->id }}" {{ $fabricante->id == $software->manufacturer_id ? 'selected' : '' }}>{{ $fabricante->name }}</option>
                    @endforeach
                </select>
        ...
        <div class="form-group">
            ...
            <input type="text" name="software_name" class="form-control" value="{{ $software->software_name }}" required>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="type" class="form-control">
                <option value="free" {{ $software->type == 'free' ? 'selected' : '' }}>Gratis</option>
                <option value="paid" {{ $software->type == 'paid' ? 'selected' : '' }}>Pago</option>
            </select>
        </div>
    ...
    $('#createFabricanteForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#createFabricanteModal').modal('toggle');
